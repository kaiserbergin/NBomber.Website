...
Before we start I'd like to inform you that you can help our project to grow and keep going just buy coffee. 
...
### Console
In the previous version, we always had a desire to improve the console output. The main pain point for us was the progress bar, which did not scale well while resizing the console, and sometimes it literally broke down when you are writing log messages to the console. The second most important problem for us was the rendering of tables, which were also displayed crookedly while resizing windows and we wanted to fix this. We started considering [Spectre.Console](https://github.com/spectreconsole/spectre.console) project for a long time as a good candidate for replacement and then started a smooth transition. It was good with tables but we still had some problems with the progress bar. Since for logs we use [Serilog](https://serilog.net/) we figure out that in order to fix our issue, we need to develop a proper integration between Serilog and Spectre.Console. After all, we developed [Serilog.Sinks.SpectreConsole](https://github.com/PragmaticFlow/Serilog.Sinks.SpectreConsole) that nicely integrates Serilog with Spectre.Console. It's open source and you can use it for your integrations too. Now our console UI is much more stable and I hope more beautiful :)

### HTML report
For rendering HTML we use [Vue.js](https://vuejs.org/) which won over us with its simplicity and minimalism. Compared to the previous version, we have significantly expanded the functionality by adding: fail stats, status codes, hints analyzer results. You can take a look at the new HTML report [here](https://serilog.net/).
...
We refactored our statistics module to fix the issue with memory growing footprint for long-running tests. Now you can run tests that will work for years and have a stable (usually small) memory footprint. This is achieved essentially due to one important optimization: all results live in memory but decomposed into buckets with the same or very similar results, and then each such bucket has its own counter, which is monotonically growing. Due to this optimization, we got a quite effective compression. Aside from this, such optimizations are very often used in time-series databases where many metrics of the same value or very similar can arrive at one point in time.

#### Default step timeout
Another important thing was introducing default timeout for step. NBomber v1 did not support any timeouts out of the box, and this task was shifted to the shoulders of the developers. For example, the HTTP plugin added its own timeout, which was quite sane. But on the other hand, everyone needs such a basic thing as a timeout, and forcing everyone to implement it is not a very good idea. Also, newbies often didn't aware of this and afterwards it leads to thread pool starvation problem. It especially was visible when testing very slow API using a big request rate, say 2-4K requests per sec from 1 node. It leads to the situation that a lot of tasks (.NET Task) were activated, consumed RAM, and never finished but growing. In the new version, each step by default contains a timeout of 1 second and can be changed by the user.

#### Removed uneccesary memory allocations
We find out a few places where we did memory allocations on every step invocation. For example, StepContext was previously created on every invocation.

#### Task computation expression
Internally, we use [FsToolkit.ErrorHandling](https://github.com/demystifyfp/FsToolkit.ErrorHandling) library that provides very useful F# computation expressions and corresponding extensions. With the latest updates to this library for work with Task, we have automatically switched to using [Ply](https://github.com/crowded/ply) library which provides a low overhead Task computation expression.
...
# New Statistics
One of the major changes we have been working on is the statistics module. Initially, we didn't really want to change it, but using NBomber v1 in production, we more and more often ran into bottlenecks or questions regarding providing advanced statistics.
...
#### Latency
For latency, we started using float instead of integer to expand the range of possible results, for example: [0.2 ms, 1.5 ms, 9.3 ms]. We were pushed to this by the case of testing a very fast in-memory database, that had a latency of less than 1 ms, and in our reports, we saw it as 0 ms instead of 0.1 - 0.5 ms. It must be admitted that the majority of load test tools represent latency as integer meaning using them you cannot cover such cases since your results will be 0 ms. 
...
#### Data transfer
For data transfer stats, we switched to using Byte instead of KB to keep parity with standard tools.
...
For certain tests, it may be necessary to understand the current step invocation count. For example, you want to change the behavior of your step execution when the step's invocation counter is reached 100 invocations. For such cases, you need to start a counter and manually incrementing it. NBomber now supports this out of the box.
...
### Dynamic step order
We decided to provide the ability to dynamically change the order of the steps at runtime. Also, it allows changing the number of steps per scenario iteration. This feature opens up a new horizon of possibilities for writing load tests. We came up with this idea from a real case when tested the database and we needed to introduce a certain randomity into our tests. For a better understanding, I suggest looking at an example:
...
### Init-only scenarios
NBomber in the very first version had the ability to provide Scenario initialization via *Scenario.init* but it had a mandatory restriction that in order to create a Scenario you should provide at least one Step.

```fsharp
// you should provide at least one step
Scenario.create "scenario" [write_step] 
...
Scenario.create "scenario_1" [write_step] 
|> Scenario.init populateKafka
...
In NBomber 2 we allow to define Scenario without steps and have only the init or clean function.

```fsharp
Scenario.create "init_scenario" [] 
|> Scenario.init populateKafka
...
Injects a random number of scenario copies (threads) per 1 sec during a given duration. Use it when you want to maintain a random rate of requests without being affected by the performance of the system under test.
...
### HTTP plugin
[NBomber.Http](https://github.com/PragmaticFlow/NBomber.Http) has been slightly modified to make it easier to use. In the previous version, this plugin provided an HttpStep wrapper over the standard Step that hides a lot of details and adds some extra magic. In the new version, we have reworked the approach with DSL plugins to use standard NBomber abstractions instead of creating new ones. Now, Http plugin contains HttpClientFactory and has an Http module with functions for building HTTP requests.
...
### InfluxDb plugin
InfluxDb plugin was supplemented with tracking fail stats and load simulation value. Now in Grafana you can render your load simulation timeline and compare it with what your target system can handle.